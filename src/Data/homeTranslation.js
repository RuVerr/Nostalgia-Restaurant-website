export const homeTranslation = {
  en: {
    linkItem: ["Home", "About Us", "Menu", "Our Contacts"],
    button: ["Reservation", "Submit", "Learn more"],
    title: [
      "Our artists",
      "Welcome to",
      "Nostalgia",
      "Menu",
      "About us",
      "Khanjyan Street, 2/1",
      "Reviews",
      "Leave your information and we will call you back."
    ],
    menuTitle: ["Hot dishes", "Barbecue (or Grilled meat)", "Appetizer", "Alcohol"],
    desc: [
      "Leave your information and we will call you back.",
      "Where taste becomes art! We cook not only dishes, but also create a unique taste.experience. Each of our dishes is made from fresh ingredients, according to author's recipes and with perfect cooking methods.",
      "Where taste becomes art! We cook not only dishes, but also create a unique taste experience. Each of our dishes is made from fresh ingredients, according to author s recipes and with perfect cooking methods",
      "All roads lead to Nostalgia Music Place in this attractive place you will feel love and art a combination of live music and luxurious interiors. Live music here creates an unforgettable mood every day, and the light and quiet music change your day.",
      "Make sure that visiting this fun and exciting place will be unforgettable, where every moment is new and unique The atmosphere is above all."
    ],
    labelTitle: ["Enter your first name", "Enter your last name", "Enter your phone number"],
    footerItem: [
      "Contact Information",
      "Address",
      "Khanjian Street, 2/1",
      "Yerevan, Armenia",
      "Phone number",
      "Working Hours",
      "Social Networks"
    ]
  },
  ru: {
    linkItem: ["Главная", "О нас", "Меню", "Наши контакты"],
    button: ["Бронирование", "Отправить", "Узнать больше"],
    title: [
      "Наши артисты",
      "Добро пожаловать в",
      "Nostalgia",
      "Меню",
      "О нас",
      "Улица Ханджян, 2/1",
      "Отзывы",
      "Контактная информация",
      "Рабочие часы",
      "Социальные сети",
      "Оставьте свои данные, и мы перезвоним вам."
    ],
    menuTitle: ["Горячие блюда", "Шашлык", "Закуски", "Алкоголь"],
    desc: [
      "Оставьте свои данные, и мы вам перезвоним.",
      "Где вкус становится искусством! Мы готовим не только блюда, но и создаём уникальный вкусовой опыт. Каждое наше блюдо приготовлено из свежих ингредиентов, по авторским рецептам и с использованием идеальных методов приготовления.",
      "Где вкус становится искусством! Мы готовим не только блюда, но и создаём уникальный вкусовой опыт. Каждое наше блюдо приготовлено из свежих ингредиентов, по авторским рецептам и с использованием идеальных методов приготовления.",
      "Все дороги ведут в Nostalgia Music Place. В этом привлекательном месте вы почувствуете любовь и искусство — сочетание живой музыки и роскошного интерьера. Живая музыка здесь создаёт незабываемое настроение каждый день, а лёгкая тихая музыка меняет ваш день.",
      "Убедитесь, что посещение этого весёлого и увлекательного места будет незабываемым, где каждый момент новый и уникальный. Атмосфера — превыше всего."
    ],
    labelTitle: ["Введите Ваше имя", "Введите фамилию", "Введите номер телефона"],
    footerItem: [
      "Контактная информация",
      "Адрес",
      "Улица Ханджян, 2/1",
      "Ереван, Армения",
      "Номер телефона",
      "Рабочие часы",
      "Социальные сети"
    ]
  },
  am: {
    linkItem: ["Գլխավոր", "Մեր մասին", "Մենյու", "Մեր կոնտակտները"],
    button: ["Ամրագրում", "Ուղարկել", "Իմանալ ավելին"],
    title: [
      "Մեր արտիստները",
      "Բարի գալուստ",
      "Nostalgia",
      "Մենյու",
      "Մեր մասին",
      "Խանջյան փողոց, 2/1",
      "Կարծիքներ",
      "Կապի տեղեկություններ",
      "Աշխատաժամեր",
      "Սոցիալական ցանցեր",
      "Թողեք ձեր տվյալները, և մենք ձեզ հետ կկապվենք։"
    ],
    menuTitle: ["Տաք ուտեստներ", "Խորոված", "Նախուտեստներ", "Ալկոհոլ"],
    desc: [
      "Թողեք ձեր տվյալները, և մենք ձեզ հետ կկապվենք։",
      "Այստեղ համը դառնում է արվեստ։ Մենք ոչ միայն պատրաստում ենք ուտեստներ, այլ նաև ստեղծում ենք յուրահատուկ համային փորձառություն։ Մեր յուրաքանչյուր ուտեստ պատրաստվում է թարմ բաղադրիչներից՝ հեղինակային բաղադրատոմսերով և կատարյալ պատրաստման եղանակներով։",
      "Այստեղ համը դառնում է արվեստ։ Մենք ոչ միայն պատրաստում ենք ուտեստներ, այլ նաև ստեղծում ենք յուրահատուկ համային փորձառություն։ Մեր յուրաքանչյուր ուտեստ պատրաստվում է թարմ բաղադրիչներից՝ հեղինակային բաղադրատոմսերով և կատարյալ պատրաստման եղանակներով։",
      "Բոլոր ճանապարհները տանում են Nostalgia Music Place։ Այս հմայիչ վայրում դուք կզգաք սեր և արվեստ՝ կենդանի երաժշտության և շքեղ ինտերիերի համադրությամբ։ Այստեղ կենդանի երաժշտությունը ամեն օր ստեղծում է անզուգական տրամադրություն, իսկ մեղմ և հանգիստ երաժշտությունը փոխում է ձեր օրը։",
      "Համոզվեք, որ այս զվարճալի և հետաքրքիր վայրը կդառնա անմոռանալի փորձառություն, որտեղ յուրաքանչյուր պահ նոր և յուրահատուկ է։ Մթնոլորտը՝ ամեն ինչից բարձր։"
    ],
    labelTitle: ["Մուտքագրեք Ձեր անունը", "Մուտքագրեք Ձեր ազգանունը", "Մուտքագրեք Ձեր հեռախոսահամարը"],
    footerItem: [
      "Կոնտակտային տեղեկություն",
      "Հասցե",
      "Խանջյան փողոց, 2/1",
      "Երևան, Հայաստան",
      "Հեռախոսահամար",
      "Աշխատանքային ժամեր",
      "Սոցիալական ցանցեր"
    ]
  }
};
